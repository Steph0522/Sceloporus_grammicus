...
metadata <- read.csv("Metadatos1.csv",check.names = F) %>% mutate(
 SampleType=case_when(SampleType=="Swab"~"Cloaca",
...
#                     c("Cloaca", "Rectum"), c("Cloaca", "Small intestine"),
#                      c("Cloaca", "Stomach"))
...
HillNumb_q2 <- ggbarplot(alpha, x= "SampleType", y= "q2",
  ...
  geom_signif(annotations=c(""), tip_length = 0.01, vjust = 0.2,
              y_position=c(48, 45),
              xmin=c(1,1), xmax=c(3,2))+ theme(axis.text.x = element_text(color = "black"))
#theme(legend.position = "none",
   #     axis.ticks.x = element_blank(),
  #      axis.text.x = element_blank())
...
q0_lme_means.t <- data.frame(q0_lme_means$contrasts)[,c(1,6)] %>% column_to_rownames(var = "contrast") %>% round(
  ...
  tab_add_title(text = paste0(
    "lme-permtest, p.value =",format(q0_lme_perm$resultats$p.value[2], digits=3, nsmall=3)) , face = "bold", padding = unit(3, "line")) %>%
...
q1_lme_means.t <- data.frame(q1_lme_means$contrasts)[,c(1,6)] %>% column_to_rownames(var = "contrast") %>% round(
  .,digits = 3) %>%replace(  is.na(.), "-")  %>% arrange(p.value) %>% rownames_to_column(var="contrast") %>% ggtexttable(
    ., rows = NULL, theme = ttheme("blank", base_size = 10)) %>%
  tab_add_title(text = paste0(
    "lme-permtest, p.value =",format(q1_lme_perm$resultats$p.value[2], digits=3, nsmall=3)) , face = "bold", padding = unit(3, "line")) %>%
  tab_add_hline(at.row = 2:3, row.side = "top", linewidth = 2)%>%
  table_cell_font(., row = 3, column = 2, face = "bold", size = 10) %>% 
  table_cell_font(., row = 4, column = 2, face = "bold",size = 10) %>% 
  table_cell_font(., row = 5, column = 2, face = "bold", size = 10) %>% 
...
q2_lme_means.t <- data.frame(q2_lme_means$contrasts)[,c(1,6)] %>% column_to_rownames(var = "contrast") %>% round(
  .,digits = 3) %>%replace(  is.na(.), "-")  %>% arrange(p.value) %>% rownames_to_column(var="contrast") %>% ggtexttable(
    ., rows = NULL, theme = ttheme("blank", base_size = 10)) %>%
  tab_add_title(text = paste0(
    "lme-permtest, p.value =",format(q2_lme_perm$resultats$p.value[2], digits=3, nsmall=3)) , face = "bold", padding = unit(3, "line")) %>%
  tab_add_hline(at.row = 2:3, row.side = "top", linewidth = 2)%>%
  table_cell_font(., row = 3, column = 2, face = "bold", size = 10) %>% 
  table_cell_font(., row = 4, column = 2, face = "bold",size = 10) %>% 
  table_cell_font(., row = 7, column = 2, face = "plain", size = 10) %>% 
